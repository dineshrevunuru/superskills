...
description: "Reverse-lookup motion glossary — turn a vague description of a web animation into its exact term ('the bouncy thing when a popover opens' → Pop in; 'the iOS rubber-band scroll' → Rubber-banding). Use when someone asks 'what's it called when…', 'what's the term for…', 'what do you call the effect where…', names an effect they can't name, or wants the right word to prompt a designer or AI with. Names an effect; does NOT design or build one (that's `craft-motion`) or decide whether it fits Dinesh's register (that's `design-taste`)."
license: "MIT — forked from `animation-vocabulary` in emilkowalski/skills (© Emil Kowalski), 'Skills For Design Engineers.' Glossary kept verbatim; house wrapper + boundaries added."
---
...
- **Use** to *name* a motion effect from a loose description — "what's it called when…", "what's the term for the thing that…", or when someone wants the exact word to prompt a designer or AI with.
- **NOT** to *design or build* the effect once named — that is `craft-motion` (choreography, stagger math, working CSS/GSAP/Framer code).
- **NOT** to *decide whether the effect belongs* in a given surface, or which motion register governs — that is `design-taste`'s motion law. This file supplies the word; it does not rule on fit.
...
**Output format:** lead with the matched term in bold, then the glossary's one-line description verbatim (see Examples). If several fit, best match first, then 1–2 alternates with a one-line note on how they differ.

## Naming ≠ endorsing (the register pointer)

This glossary is **register-neutral**: it names the restraint-register terms (the *Entrances & Exits* and *Easing* clusters) and the physics/gesture terms (the whole *Spring Animations* block, plus *Drag*, *Swipe to dismiss*, *Rubber-banding*, *Momentum*, *Velocity*, *Interruptible animation*) with equal footing. Naming a spring is not endorsing it.

- **Whether a named effect fits Dinesh, and which register governs, is `design-taste`'s call — never restated here.** The physics/gesture cluster legitimately describes *direct-manipulation* surfaces (drag, bottom sheets, swipe, voice barge-in/duck); on those a spring is not wrong, and it is not this file's job to flag it. Content-presentation motion runs a different, restraint-first register. Which one applies → `design-taste`; how to build it → `craft-motion`.
- **Universal — holds in both registers:** the *Easing* rules (ease-out for enter/exit, avoid ease-in on UI) and the *Performance* rules (compositing, `will-change`, hardware acceleration — animate transform + opacity on the GPU) are baseline craft that applies everywhere. Ease-out points the **same way** as `design-taste`'s settle curve; GPU-only is `craft-motion`'s baseline technique. Read them as reinforcement, not a rival system.
- **Register-gated — the exception, never the default:** only *spring / overshoot* (the *Spring Animations* block, plus *Pop in* and *Bounce*) is confined to the gesture register. It is correct under direct manipulation and out of the presentation default — `design-taste`'s 0%-overshoot law still governs everything system-driven. Naming a spring is never endorsing it on a presentation surface.
- Durations, curve values, and duration tiers live in `design-taste`, not here. This file names the *effect*, never its timing.
...
_Kept verbatim from Emil Kowalski's animation vocabulary — authoritative; quote, don't paraphrase._
...
| Don't | Do |
|---|---|
| Invent a term because the description almost fits one | Name the closest match and say plainly it's an approximation |
| Paraphrase a glossary line into your own words | Quote it verbatim — the descriptions are the authority |
| Answer with the technical name from the description's keywords | Read for the *sensation* the user felt, then map it |
| Rule "that's a bad idea / too bouncy" when asked for a name | Just name it; fit and register are `design-taste`'s call |
| Flag a spring as wrong because it overshoots | On a drag/sheet/swipe surface a spring is legitimate — name it, don't judge |
| Bury the answer in a paragraph of theory | Lead with the bold term; expand only if asked |

## Boundaries

- **`craft-motion`** owns designing and building the effect once it's named — choreography, stagger math, and working CSS/GSAP/Framer code. This skill hands off the word; that skill materializes it.
- **`design-taste`** owns whether a named effect belongs and which motion register governs (the restraint-first default for content presentation vs. the gesture register for direct-manipulation surfaces), plus all curve/duration values. This file names effects register-neutrally and never restates a taste value.
- **`craft-critique`** owns the verdict on finished motion; a naming answer is not a judgment.

## Sources

- Forked from **`animation-vocabulary`** — emilkowalski/skills, MIT (© Emil Kowalski), "Skills For Design Engineers." The glossary and the reverse-lookup method are kept as-authored; the glossary text is preserved verbatim. Added for this library: the trigger-rich description, the `When to use / when NOT to use` split, the register pointer (naming ≠ endorsing), the Don't/Do table, and the `Boundaries` to `craft-motion` / `design-taste` / `craft-critique`.
